...
namespace FrameSoftWeb.Controllers
{
    public class ReporterController : Controller
    {
        ...
        static System.Text.StringBuilder errors = new System.Text.StringBuilder();
        ...
        public ActionResult ReportApplicationError()
        {
            try
            {
                ...
                if (version == "2.1.1.1" || version == "2.1.1.2" || version == "2.1.1.3" || version == "2.1.1.4" || version == "2.1.1.5" || version == "2.1.1.6")
                    return Content("nok");

                var guid = headers["Guid"];
                var guidId = Engine.Amar.FramesoftServiceProvider.GetAndUpdateGuidIDByGuid(Guid.Parse(guid));
                ...
                errors.AppendLine("New Start=>");
                errors.AppendLine(osVersion);
                errors.AppendLine(version);
                errors.AppendLine(headers["ErrorMessage"]);
                using (StreamReader reader = new StreamReader(Request.InputStream))
                {
                    errors.AppendLine(reader.ReadToEnd());
                }
            ...
            {
                errors.AppendLine("**** Server Error ***" + e.Message + "\r\n" + e.StackTrace);
                return Content(e.Message);
            }
        }

        public ActionResult ClearApplicationErrors()
        {
            errors.Clear();
            return Content("OK");
        }
