                ...
                try
                {
                    ...
                    if (fromProperty.CanWrite && obj1 != null)
                    {
                        ...
                        if (MatchingProps(fromProperty, destinationProperty))
                        {
                            ...
                            {
                                if (val is IList)
                                    newVal = typeof(Enumerable).GetMethod("ToList", BindingFlags.Static | BindingFlags.Public).Invoke(null, new object[] { val });
                                else
                                    newVal = val;
                            ...
                            if (manualMapProperties != null && manualMapProperties.Contains(fromProperty.Name))
                            {
                                ...
                                try
                                {
                                    if (getValue is IList)
                                        getValue = typeof(Enumerable).GetMethod("ToList", BindingFlags.Static | BindingFlags.Public).Invoke(null, new object[] { getValue });
