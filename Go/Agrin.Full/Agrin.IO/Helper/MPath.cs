...
namespace Agrin.IO.Helper
{
    ...
    {
        public static Action InitedAction { get; set; }
        public static void InitializePath(string applicationDirectoryPath = null, string downloadsPath = null, string downloadSaveDataPath = null)
        {
            ...
            try
            {
                if (!Directory.Exists(CurrentUserAppDirectory))
                    CrossDirectoryInfo.Current.CreateDirectory(CurrentUserAppDirectory);
            }
            catch (Exception e)
            {
            ...
            try
            {
                if (!Directory.Exists(SaveDataPath))
                    CrossDirectoryInfo.Current.CreateDirectory(SaveDataPath);
            }
            catch (Exception e)
            {
            ...
            try
            {
                if (!Directory.Exists(RepairSaveDataPath))
                    CrossDirectoryInfo.Current.CreateDirectory(RepairSaveDataPath);
            }
            catch (Exception e)
            {
        ...
        public static string CurrentUserAppDirectory
        {
            ...
            set
            {
                ...
                BackUpMixersPath = Path.Combine(value, "Mixers");
            }
        }

        static string _backUpCompleteLinksPath = null;
        /// <summary>
        /// محل ذخیره ی بک آپ ها جهت ترمیم لینک ها
        /// </summary>
        public static string BackUpCompleteLinksPath
        {
            get
            {
                if (_backUpCompleteLinksPath != null && !Directory.Exists(_backUpCompleteLinksPath))
                {
                    try
                    {
                        CrossDirectoryInfo.Current.CreateDirectory(_backUpCompleteLinksPath);
                    }
                    catch (Exception e)
                    {
                        Agrin.Log.AutoLogger.LogError(e, "IOHelperBase.Current.CreateDirectory(_backUpCompleteLinksPath)");
                    }
                }
        ...
        static string _backUpMixersPath = null;
        public static string BackUpMixersPath
        {
            get
            {
                if (_backUpMixersPath != null && !Directory.Exists(_backUpMixersPath))
                {
                    try
                    {
                        CrossDirectoryInfo.Current.CreateDirectory(_backUpMixersPath);
                    }
                    catch (Exception e)
                    {
                        Agrin.Log.AutoLogger.LogError(e, "IOHelperBase.Current.CreateDirectory(_backUpMixersPath)");
                    }
                }
                return _backUpMixersPath;
            }
            set
            {
                _backUpMixersPath = value;
            }
        }
        ...
        public static string DownloadsPath
        {
            get
            {
                if (!Directory.Exists(_downloadsPath))
                    CrossDirectoryInfo.Current.CreateDirectory(_downloadsPath);
                return _downloadsPath;
            }
            set { _downloadsPath = value; }
        }

        static string _SecurityPath = "";
        ...
        public static string GetMimeTypeFromExtension(string ext)
        {
            ...
            while (key != null)
            {
                ...
                if (name == null)
                    return ext + " File";
                Microsoft.Win32.RegistryKey tempKey = Microsoft.Win32.Registry.ClassesRoot.OpenSubKey(name);
        ...
        static string androidinvalidChars = "|\\?*<\":>+[]/'";
#endif
        ...
        public static string GetFileName(Uri uri)
        {
            ...
            if (fname.Length > 0)
            {
                ...
                if (String.IsNullOrEmpty(MPath.GetFileExtention(fname)))
        ...
        public static void CreateDoesNotExistDirectory(string dir)
        {
            try
            {
                ...
                {
                    CrossDirectoryInfo.Current.CreateDirectory(dir);
                }
            }
            catch (Exception e)
            {
