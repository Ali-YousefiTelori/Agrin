    ...
    public static class AsyncActions
    {
        ...
        public static void Action(Action action, Action<Exception> errorReport, bool isThread)
        {
            if (isThread)
            {
                ...
                thread = new Thread(() =>
                {
                    ...
                    {
                        RunErrorReport(errorReport, e);
                        Agrin.Log.AutoLogger.LogError(e, "RunErrorReport 0", true);
                    }
                    lock (addremoveLock)
        ...
        static void RunErrorReport(Action<Exception> errorReport, Exception e)
        {
            if (errorReport != null)
            {
                ...
                {
                    Agrin.Log.AutoLogger.LogError(c, "RunErrorReport 1", true);
                }
            }
            else
                Agrin.Log.AutoLogger.LogError(e, "RunErrorReport 2", true);
        }
    }
}
