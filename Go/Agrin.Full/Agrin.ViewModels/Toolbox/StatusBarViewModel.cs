    ...
    public class StatusBarViewModel : StatusBarBaseViewModel
    {
        ...
        public StatusBarViewModel()
        {
            ...
            TimeDownloadEngine.TotalItemsChanged = (totalSpeed, totalMaximumProgress, totalProgressValue, isConnecting, isError, complete) =>
            {
                ...
                {
                    isNormal = false;
                    TaskbarProgress.SetState(System.Windows.Shell.TaskbarItemProgressState.Paused);
                }
                else if (isError)
                {
                    isNormal = false;
                    TaskbarProgress.SetState(System.Windows.Shell.TaskbarItemProgressState.Error);
                }
                else
                {
                    if (!isNormal)
                        TaskbarProgress.SetState(System.Windows.Shell.TaskbarItemProgressState.Normal);
                    isNormal = true;
                }
