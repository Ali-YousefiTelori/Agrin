...
namespace Agrin.BaseViewModels.Help
{
    public class FeedBackBaseViewModel : ANotifyPropertyChanged
    {
        ...
        public void SendMessage()
        {
            ...
            AsyncActions.Action(() =>
            {
                var send = Agrin.About.SendMessage.SendFeedBack(new About.UserMessage() { GUID = ApplicationSetting.Current.ApplicationOSSetting.ApplicationGuid, message = Message, LastUserMessageID = ApplicationSetting.Current.LastUserMessageID, SystemIMEI = ApplicationSetting.Current.ApplicationOSSetting.SystemIMEI });
                if (send)
                {
