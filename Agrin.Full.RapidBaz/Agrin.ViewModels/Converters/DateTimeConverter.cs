...
namespace Agrin.ViewModels.Converters
{
    public class DateTimeConverter : IValueConverter
    {
        ...
        static string GetPersian(DateTime dateTime, bool isString)
        {
            ...
            return cal.GetYear(dateTime) + "/" + cal.GetMonth(dateTime) + "/" + cal.GetDayOfMonth(dateTime) + " " + String.Format("{0:00}:{1:00}", dateTime.Hour, dateTime.Minute);
        }
            ...
            if (value is DateTime && DateTime.Now >= (DateTime)value)
            {
                if (IsTextMode)
                {
                    try
                    {
                        ...
                        if (ShowTime)
                        {
                            str.Append(" ساعت: " + String.Format("{0:00}:{1:00}",  ((DateTime)value).Hour, ((DateTime)value).Minute));
                        }
                        return str.ToString();
                    }
                    catch
