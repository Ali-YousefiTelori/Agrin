...
namespace Agrin.Download.Data
{
    ...
    public static class SerializeData
    {
        ...
        static VirtualSaver LinkInfoesToFileVirtualSaver = new VirtualSaver(), ApplicationTaskToFileVirtualSaver = new VirtualSaver(),
           AppServiceDataVirtualSaver = new VirtualSaver(), ApplicationSettingToFileVirtualSaver = new VirtualSaver(),
           ...
           SaveNotificationToFileVirtualSaver = new VirtualSaver(), ApplicationIPsDataVirtualSaver = new VirtualSaver();
        ...
        public static void SaveLinkInfoDataToFile(LinkInfo info, string fileName = "Link.agn", string address = null, bool force = false)
        {
            ...
            VirtualSaveToFile(saver, () =>
            {
                ...
                convert = (mapName, mapType) =>
                {
                    ...
                    {
                        if (mapType is FastCollection<LinkWebRequest>)
                        {
                            List<ConnectionInfoSerialize> ret = new List<ConnectionInfoSerialize>();
                            foreach (var item in ((FastCollection<LinkWebRequest>)mapType).ToList())
                            {
        ...
        public static void SaveNotificationToFile()
        {
            VirtualSaveToFile(SaveNotificationToFileVirtualSaver, () =>
            {
                ...
                convert = (mapName, mapType) =>
                {
                    if (mapName == "Items")
                    {
                        if (mapType is FastCollection<LinkInfo>)
                        {
                            List<int> ret = new List<int>();
                            foreach (var item in ((FastCollection<LinkInfo>)mapType).ToList())
                            {
        ...
        public static void SaveGroupInfoesToFileNoThread()
        {
            ...
            {
                saveLists.Add(new GroupInfoSerialize() { Name = item.Name, SavePath = item.SavePath, Extentions = item.Extentions, Id = item.Id, IsExpanded = item.IsExpanded });
            }
