...
{
    public class RadioJavanConnectionInfo : AConnectionInfo
    {
        public RadioJavanConnectionInfo(string address, LinkWebRequest parentLinkWebRequest)
        {
        ...
        public override bool CreateRequestData()
        {
            try
            {
                ...
                if (RadioJavanFindDownloadLink.IsRadioJavanLink(address.Address))
                {
                    if (ParentLinkWebRequest.Parent.DownloadingProperty.UriCache == null)
                    {
                        ...
                        var radioJavanLink = RadioJavanFindDownloadLink.FindLinkFromSite(normalURL, proxy);

                        if (isDispose)
                            return false;
                        if (ParentLinkWebRequest.Parent.Management.SharingSettings.Count == 1)
                        {
                            var info = RadioJavanFindDownloadLink.GetQualityByText(radioJavanLink.Qualities, ParentLinkWebRequest.Parent.Management.SharingSettings.First().ToString());
                            ParentLinkWebRequest.Parent.PathInfo.AddressFileName = Agrin.IO.Helper.MPath.GetFileNameValidChar(radioJavanLink.Title + MPath.GetFileExtention(info.Link));
                            uri = info.Link;
                        ...
                        else
                        {
                            ...
                            {
                                RadioJavanQualityInfo info = item as RadioJavanQualityInfo;
                                uri = info.Link;
                                _currentAddress = uri;
                                ParentLinkWebRequest.Parent.DownloadingProperty.UriCache = uri;
                            ...
                            ParentLinkWebRequest.Parent.Management.UserMustSelectItemAction(radioJavanLink.Qualities.Cast<object>().ToList());
