...
namespace Agrin.ViewModels.Converters
{
    public class DateTimeConverter : IValueConverter
    {
        ...
        {
            if (isString)
                return ApplicationHelperBase.GetAppResource(cal.GetDayOfWeek(dateTime) + "_Language") + " " + cal.GetYear(dateTime) + "/" + cal.GetMonth(dateTime) + "/" + cal.GetDayOfMonth(dateTime) + " " + ApplicationHelperBase.GetAppResource("Hour_Language") + " " + dateTime.Hour + ":" + dateTime.Minute;
            ...
            if (value is DateTime && DateTime.Now >= (DateTime)value)
            {
                if (IsTextMode)
                {
                    try
                    {
                        ...
                        int day = DateTime.Today == ((DateTime)value).Date ? 0 : dt.Day;
                        if (year > 0)
                        {
                        ...
                        {
                            if (items.Count > 0)
                                items.Add(" " + ApplicationHelperBase.GetAppResource("And_Language") + " ");
                        ...
                        if (ShowTime)
                        {
                            str.Append(" ساعت: " + ((DateTime)value).Hour + ":" + ((DateTime)value).Minute);
                        }
                        return str.ToString();
                    }
                    catch
