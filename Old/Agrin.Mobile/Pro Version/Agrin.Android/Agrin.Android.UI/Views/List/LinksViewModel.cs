            ...
            try
            {
                ...
                else if (menu.Mode == MenuItemModeEnum.GenerateAutoMixerReport)
                {
                    ...
                    foreach (Download.Web.Link.LinkWebRequest item in Download.Helper.LinkHelper.SortByPosition(info.Connections))
                    {
                        ...
                        build.Append(item.ConnectionId + ":" + item.StartPosition + "," + item.EndPosition + "-");
                    }
                    build.Append(System.Environment.NewLine);
                    build.AppendLine("آدرس های لینک:");
                    foreach (var item in info.Management.MultiLinks)
                    {
                        build.AppendLine(item.Address);
                    }
                    build.Append(System.Environment.NewLine);
                    string report = build.ToString();
                    if (files.Count == 0)
                    ...
                    try
                    {
                        System.IO.File.WriteAllText(System.IO.Path.Combine(Agrin.Log.AutoLogger.ApplicationDirectory, "AgrinLinkInfoReport.txt"), report, Encoding.UTF8);
                        System.IO.File.WriteAllText(System.IO.Path.Combine(Agrin.Log.AutoLogger.ApplicationDirectory, "FullAgrinLinkInfoReport.txt"), Newtonsoft.Json.JsonConvert.SerializeObject(info), Encoding.UTF8);
                    }
                    catch (Exception ex)
                    {
                        AutoLogger.LogError(ex, "GenerateAutoMixerReport 2");
