...
namespace Agrin.BaseViewModels.Toolbox
{
    ...
    public class StatusBarBaseViewModel : ANotifyPropertyChanged
    {
        public StatusBarBaseViewModel()
        {
            ...
            UserManager.ChangedUserInfo = (rapidUserInfo) =>
            {
                try
                {
                    UploadedFromRapidBaz = rapidUserInfo.UploadSize;
                    long total = long.Parse(rapidUserInfo.TotalSize) * 1024 * 1024;
                    long up = long.Parse(rapidUserInfo.UploadSize) * 1024 * 1024;
                    RapidBazRemainingSize = total - up;
                }
                catch
                {
        ...
        public FastCollection<StatusBarItem> SpeedLimiterItems
        {
            get
            {
                if (_SpeedLimiterItems == null)
                    _SpeedLimiterItems = new FastCollection<StatusBarItem>(ApplicationHelperMono.DispatcherThread);
        ...
        public FastCollection<StatusBarItem> ConnectionCountItems
        {
            get
            {
                if (_ConnectionCountItems == null)
                    _ConnectionCountItems = new FastCollection<StatusBarItem>(ApplicationHelperMono.DispatcherThread);
        ...
        private string _UploadedFromRapidBaz = "0";

        public string UploadedFromRapidBaz
        {
            get { return _UploadedFromRapidBaz; }
            set
            {
                _UploadedFromRapidBaz = value;
                OnPropertyChanged("UploadedFromRapidBaz");
            }
        }

        private long _RapidBazRemainingSize = 0;

        public long RapidBazRemainingSize
        {
            get { return _RapidBazRemainingSize; }
            set
            {
                _RapidBazRemainingSize = value;
                OnPropertyChanged("RapidBazRemainingSize");
            }
        }
