        ...
        public string LastErrorDescription
        {
            get
            {
                if (Errors.Count > 0)
                {
                    ...
                    if (Parent != null && Parent.PathInfo.Address.Contains("rapidbaz.com") && error.ExceptionData is WebException && error.OtherData.Count >= 2 && error.OtherData[1] is HttpStatusCode
                        && ((HttpStatusCode)error.OtherData[1] == HttpStatusCode.InternalServerError || (HttpStatusCode)error.OtherData[1] == HttpStatusCode.NotAcceptable))
                    {
                        return "سرور مشغول است لطفاً بعداً مجدداً سعی کنید.";
                    }
#if(MobileApp)
