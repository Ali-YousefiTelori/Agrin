...
namespace Agrin.Download.Web
{
    ...
    public class NotificationInfo : ANotifyPropertyChanged
    {
        public string Text
        {
            get
            {
                if (Mode == NotificationMode.Complete)
                    return ApplicationHelperMono.GetAppResource("Complete_Language");
                else
                    return ApplicationHelperMono.GetAppResource("ErrorNotify_Language");
            }
        }
        ...
        public FastCollection<LinkInfo> Items
        {
            ...
            {
                if (_Items == null)
                    _Items = new FastCollection<LinkInfo>(ApplicationHelperMono.DispatcherThread);
                return _Items;
            }
            set { _Items = value; }
        }
